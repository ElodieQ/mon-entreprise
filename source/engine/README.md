...
une ligne explicite `taux: 5%` par une référence vers une règle tierce `taux: taux`, qui est loin d'être aussi claire.

Pour ce cas d'usage il est possible d'utiliser un **paramètre nommé**. On garde
la définition de la prime inchangée et on annote avec une flèche `~>` l'argument
auquel on veut accéder depuis l'extérieur :
...
Par défaut le paramètre nommé est référencé son nom dans l'espace de nom de la
...
Lors d'une relecture future de la règle `prime` la flèche `~>` indique que du
code extérieur dépend du paramètre `taux`, ce a quoi il faut être vigilant en
cas de ré-écriture.
